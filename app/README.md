...
基于 RAG（检索增强生成）的医学影像报告结构化输出系统，通过向量检索 + Rerank 精排 + LLM 生成，将用户输入的检查信息转化为规范的影像学报告。
...
用户输入 "CT脑出血"
       ...
       ▼
┌─────────────────┐
│  bge-m3 向量化  │  问题 → 1024维查询向量
└───────┬─────────┘
        │
        ▼
┌─────────────────┐
│  Milvus 向量检索 │  余弦相似度匹配 → Top-K 候选（粗筛）
└───────┬─────────┘
        │
        ▼
┌─────────────────┐
...
- **Rerank 精排**：向量检索召回 Top-K 候选后，调用 Qwen3-VL-Reranker-8B 进行 Cross-Encoder 精排，返回带相关性分数的候选
...
用户输入 → 向量检索 → Rerank 精排 → LLM 判断生成，四步完成：

```
用户输入 "CT脑出血"
       │
       ▼
① 向量检索 top-5（bge-m3 + Milvus 余弦匹配）
   → 返回5条候选，按余弦相似度排序
       │
       ▼
② Rerank 精排 top-3（Qwen3-VL-Reranker-8B 交叉评分）
   → 对5条候选逐一与问题做 Cross-Encoder 评分
   → 按分数重新排序，取前3条
       │
       ▼
③ 构造 Prompt（拼接3条参考 + Rerank分数 + 用户问题）
   → LLM 收到带分数的参考信息
       │
       ▼
④ LLM 判断+生成
...
> 📖 完整流程详解：[Text_Embedding.md](markdown_files/Text_Embedding.md) — 数据入库6步 + 报告生成4步，含代码对应和示例
...
| 文档                                                  | 内容                                                                      |
| ----------------------------------------------------- | ------------------------------------------------------------------------- |
| [Text_Embedding.md](markdown_files/Text_Embedding.md) | 数据入库流程（6步详解）+ 报告生成流程（4步详解）+ 向量检索 vs Rerank 对比 |
