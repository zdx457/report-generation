...
基于 RAG（检索增强生成）的医学影像报告结构化输出系统，通过查询改写 + 多路召回 + Rerank 精排 + LLM 生成，将用户输入的检查信息转化为规范的影像学报告。
...
用户输入 "CT脑出血"
       ...
       ▼
┌──────────────────────┐
│  模糊检测 & 查询改写  │  过于模糊 → 追问用户；不够具体 → LLM 扩展查询词
└──────────┬───────────┘
           │
           ▼
┌──────────────────────┐
│  多路召回（3路并行）   │
│  ├ 向量检索（语义匹配）│  bge-m3 → 1024维 → Milvus 余弦相似度
│  ├ 元数据过滤（精确）  │  检查类型/部位/诊断结论精确匹配
│  └ 关键词检索（全文）  │  Milvus like 全文匹配
│  → 合并去重 → 候选集  │
└──────────┬───────────┘
           │
           ▼
...
- **查询改写**：当用户输入不够具体时（如"CT头颅"），通过 LLM 扩展为更完整的检索描述
...
├── query_rewrite.py     # 查询改写模块（模糊检测 + LLM 改写 + 追问）
...
用户输入 → 模糊检测/查询改写 → 多路召回 → Rerank 精排 → 拼接提示词 → LLM 生成，六步完成：
...
① 用户输入 & 模糊检测
   ...
   → 不够具体（如"CT头颅"）→ LLM 改写为更完整的检索描述
   → 足够具体 → 直接使用原始查询
       │
       ▼
② 多路召回 top-K（每路独立检索，合并去重）
   ├ 路径1: 向量检索（bge-m3 + Milvus 余弦匹配）→ 语义相似度候选
   ├ 路径2: 元数据过滤（检查类型/部位/诊断精确匹配）→ 精确匹配候选
   └ 路径3: 关键词检索（Milvus like 全文匹配）→ 全文匹配候选
   → 三路合并去重 → 候选集
       │
       ▼
...
| 文档                                                  | 内容                                                                      |
| ----------------------------------------------------- | ------------------------------------------------------------------------- |
| [Text_Embedding.md](markdown_files/Text_Embedding.md) | 数据入库流程（6步详解）+ 报告生成流程（6步详解）+ 向量检索 vs Rerank 对比 |
