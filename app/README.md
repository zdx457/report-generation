...
- **缺少模态拦截**：当用户输入只有部位/诊断但没有检查类型时（如"脑出血"），自动追问用户补充 CT/MR 等，避免生成不完整报告
...
| 自动切片   | 上传后自动将 xlsx 每一行按列头切分为独立 Markdown 文件，命名格式为"检查类型*部位*检查项目\_诊断" | `xlsx_slicer.py`                |
